...
export function ChatBot({
  ...
  const generateStatsResponse = (analysis: AnalysisResult): ChatMessage => {
    ...
    const completionRate =
      data.vehicles > 0 ? ((data.completedToday / data.vehicles) * 100).toFixed(1) : "0.0";
      ...
      actions:
        pendingVehicles.length > 0
          ? [
              {
                type: "bulk-operation",
                label: "⚡ Μαζική Ενημέρωση",
                data: { type: "update-pending" },
              },
  ...
  const generateShiftsResponse = (_query: string): ChatMessage => {
    ...
    if (todayShifts.length > 0) {
      const shiftsByType = todayShifts.reduce(
        (acc, s) => {
    ...
    const todayReservations = reservations.filter(
      (r) => r.reservationDate.toDateString() === today.toDateString()
    ).length;
  ...
  const generateStaffResponse = (_query: string): ChatMessage => {
    ...
    if (activeStaff.length > 0) {
      const roleCount = activeStaff.reduce(
        (acc, s) => {
    ...
    if (todayWork.length > 0) {
      const technicianStats = todayWork.reduce(
        (acc, w) => {
      ...
      const topPerformer = Object.entries(technicianStats).sort(
      ...
      actions:
        pendingLeaveRequests.length > 0
          ? [
              {
                type: "suggestion",
                label: "✅ Έγκριση Αδειών",
                data: { action: "approve-leaves" },
              },
              {
                type: "analysis",
                label: "📊 Απόδοση Προσωπικού",
                data: { type: "staff-performance" },
              },
            ]
          : [
              {
  ...
  const generateReservationsResponse = (_query: string): ChatMessage => {
    ...
    if (todayReservations.length > 0) {
      const statusCount = todayReservations.reduce(
        (acc, r) => {
      ...
      // Time slot analysis
      const timeSlots = todayReservations.reduce(
        (acc, r) => {
    ...
    const utilizationRate =
      capacity > 0 ? ((todayReservations.length / capacity) * 100).toFixed(1) : "0.0";
  ...
  const generatePredictionsResponse = (analysis: AnalysisResult): ChatMessage => {
    ...
    const avgRevenue =
      todayWork.length > 0 ? todayWork.reduce((sum, w) => sum + w.cost, 0) / todayWork.length : 0;
  ...
  const generateReportsResponse = (): ChatMessage => {
    ...
    return {
      ...
      actions: [
        {
          type: "report",
          label: "📥 Ημερήσια Αναφορά (Excel)",
          data: { type: "daily", format: "excel" },
        },
        {
          type: "report",
          label: "📥 Εβδομαδιαία Αναφορά (PDF)",
          data: { type: "weekly", format: "pdf" },
        },
  ...
  const generateHelpResponse = (): ChatMessage => {
    ...
    return {
      ...
      actions: [
        {
          type: "suggestion",
          label: "📊 Δοκίμασε: Στατιστικά",
          data: { query: "Δείξε μου στατιστικά" },
        },
        {
          type: "suggestion",
          label: "🚗 Δοκίμασε: Οχήματα",
          data: { query: "Ποια οχήματα εκκρεμούν" },
        },
        {
          type: "suggestion",
          label: "🔮 Δοκίμασε: Προβλέψεις",
          data: { query: "Πρόβλεψε τα έσοδα" },
        },
      ],
    };
  };
